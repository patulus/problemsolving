...
public class Main {
    ...
    private static void dfs(StringBuilder sb, int N, int M, int[] history, boolean[] visited, int depth) {
        if (depth == M) {
            for (int i = 0; i < M; i++) {
        ...
        for (int i = 1; i <= N; i++) {
            if (!visited[i]) {
                visited[i] = true;
                history[depth] = i;
                dfs(sb, N, M, history, visited, depth + 1);
                visited[i] = false;
            }
        }
    }
